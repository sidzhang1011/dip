...
%   X = HUFF2MAT(Y) decodes a Huffman encoded structure Y with uintl6
%   fields:
...
if ~isstruct(y) || ~isfield(y, 'min') || ~isfield(y, 'size') || ...
    ...
    error('The input must be a structure as returened by MAT2HUFF.');
end

sz = double(y.size);    m = sz(1); n = sz(2);
xmin = double(y.min) - 32768;           % Get x mininum
...
% plus '0' and '1' or addresses (-) to decoded Huffman codewords in 'map'.
% Array 'left' is a list of nodes yet to be prorcessed for 'link' entries.
