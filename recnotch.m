...
%   H = RECNOTCH(NOTCH, MODE, M, N, W, SV, SH) generates an M-by-N notch
%   filter consisting of symmetric pairs of rectangles of width W placed on
%   the vertical and horizontal axes of the (centered) frequency rectangle.
%   The vertical rectangles start at +SV and -SV on the vertical axis and
%   extend to both ends of the axis. Horizontal rectangles silimlarly start
%   at +SH and -SH and extend to both ends of the axis. These values are
%   with respect to the origin of the axes of the centered frequency
%   rectangles. For example, specifying SV = 50 creates a rectangle of
%   width W that starts 50 pixels above the center of the vertical axis and
%   extends up to the first row of the filter. A similar rectangle is
%   created starting 50 pixels below the center and extending to the last
%   row. W must be an odd number to preserve the symmetry of the filtered
%   Fourier transform.
%
...
%
%       One of these thress values must be specified in the call.
%
...
% horizontal rectangles: 0 for notchrej ect and 1 for notchpass .
% Filters are computed initially as reject filters and then changed % to pass if so specified in NOTCH.
if strcmp(mode, 'both')
    AV = 0;
    AH = 0;
elseif strcmp (mode , 'horizontal') 
    AV = 1 ; % No rej ect filtering 
    AH = 0;
...
% Begin filter computation . The filter is generated as a reject filter. At
% the end, it are changed to a notchpass filter if is is so specified in
% parameter NOTCH.
H = rectangleReject(M, N, W, SV, SH, AV, AH);

% Finished computing the rectangle notch filter. Format the output.
