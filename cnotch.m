...
%   domain coordinates (u,v) taht define the centers of the filter
%   notches (when specifying filter locations, remember that
%   coordinates in MATLAB run form 1 to M and 1 to N). Coordinates
%   (u, v) are specified for one notch only. The corresponding symmetric
%   notches are generated automatically. D0 is the radius (cut-off
%   frequency) of the notches. It can be specified as a scalar, in which
%   case it is used in all K notch pairs, or it can be a vector of length
%   K, containing an individual cutoff value for each notch pair. n is the
%   order of the Butterworth filter if one is specified.
%
...
%   consistentency with filtering function dftfilt. To  view H as an image
...
% required for preserve symmetry in the filter. If necessary, a row and/or
% column is added to the filter at the end of the function.
M0 = M;
N0 = N;
...
% Shift notch centers so that they are with respect to the center of the
% filter (and the frequency rectangle).
center = repmat(center, size(C,1), 1);
% size(center)
C = C - center;

% Begin filter computations. All filters are computed as notchreject
% filters. At the end, they are changed to notchpass filters if it is so
...
% Initialize the filter array to be an "all pass " filter. This constant
% filter is then multiplied by the notchreject filters placed at the
% location in C with respect to the center of the frequency rectangle.
H = ones(M0, N0, 'single');

% Generate filter.
for I = 1:K
    % Place a notch at each location in delta. Function hpfilter returns
    % the filters uncentered. Use fftshift to center the filter at each
    % location. The filters are made larger than M-by-N  to simplify
    % indexing in function placeNotches.
...
% The following calculations are to determine the (common) area of overlap
% between array H and the notch filter FILT.
if delu >=0 && delv >= 0
    filtCommon = filt(1:M, 1:N);    % Displacement is in Q1.
elseif delu < 0 && delv >= 0
    filtCommon = filt(U+1: U+M, 1:N);   % Displacement is in Q2.
elseif delu < 0 && delv < 0
    filtCommon = filt(U+1 : U+M, V+1 : V+N);    % Displacement is in Q3.
elseif delu >=0 && delv <= 0
...
% At this point, H is an odd array in both dimensions (see comments at the
% beginning of the function). In the following, we insert a row if M is
% even, and a column if N is even. The new row and column have to be
% symmetric about their center to preserve symmetry in the filter. They are
% created by duplicating the first row and column of H and then making them
% symmetric.
centerU = center(1, 1);
centerV = center(1, 2);
newRow = H(1, :);
