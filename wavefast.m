...
%   [C, S] = WAVEFAST(X, N, LP, HP) computes 'PAGES' 2D N-level FWTs of a
%   'ROWS x COLUMNS x PAGES' matrix X with respect to decomposition filters
%   LP and HP. 
%
%   [C, S] = WAVEFAST(X, N, WNAME) performs the same operation but fetches
...
%   distortion, X is symmetrically extended. That is, if X = [c1 c2 c3 ...
%   cn] (in 1D),  then its symmetric extension would be [... c3 c2 c1 c1 c2
%   c3 ... cn cn cn-1 ...].
%
%   OUTPUTS:
...
%             d1(n) ... dk(n) h1(n-1) ... hk(n-1) .... dk(1) ]
%
%       where ai, hi, vi, and di for i = 0,1,...k are columnwise vectors
%       containing approximation, horizontal, vertical, and diagonal
%       coefficient matrices, respectively, and k is the number of pages in
%       the 3-D extended array X. C has 3n + 1 sections where n is the
...
% Check the input arguments for reasonbleness.
...
for i = 1:n
    ...
    % Convolve rows with LP and downsample. Then convolve columns with 
    % HP and LP to get the horizontal and next approximation coeffcients.
...
% downsample, and extract the center section since symmetrically extended.
