...
%   Huffman-encoded data and additional parameters needed by 3PEG2K2IM
%   for decoding. 
%
%   See also 1PEG2K2IM.
...
% Run-length code zero runs of more than 10.  Begin by creating a special
% code for 0 runs ('zrc) and end-of-code ('eoc') and making a run-length
% table.
zrc = min(c(:)) - 1;        eoc = zrc - 1;

% The RUNS variable is shared with the RUNS variable in the nested function
% runcode. See Chapter 3 of DIPUM3E for a discussion of nested functions.
RUNS = 65535;

% Find the run transition points: 'plus' contains the index of the start of
% a zero run; the corresponding 'minus' is its end + 1.
...
q = round(q*100)/100;               % Round to l/100th place
if any(100*q > 65535)
