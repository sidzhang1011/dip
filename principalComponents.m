...
%   vectors of the vector population contained in the rows of X, a matrix
%   of size K-by-n where K (assumed to be > 1) is the number of vectors and
%   n is their dimensionality. Q, with values in the range [0,n], is the
...
%
%     P.Y       K-by-Q matrix whose columns are the principal-component
%               vectors.
%     P.A       Q-by-n principal components transformation matrix whose
%               rows are the Q eigenvectors of Cx corresponding to the Q
%               largest eigenvalues.
...
%     P.Cy      The Q-by-Q covariance matrix of the population in Y. The
%               main diagonal contains the eigenvalues (in descending
%               order) corresponding to the Q eigenvectors.
...
% Sort the eigenvalues in decreasing order. Rearrrange the eigenvectors to
% match.
d = diag(D);
[d, idx] = sort(d);
d = flipud(d);
...
% The mean square error is the sum of all the eigenvalues minus the sum of
